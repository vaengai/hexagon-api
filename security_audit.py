...
def check_file_security():
    ...
    try:
        ...
        if tracked_env:
            print(
                f"❌ SECURITY ISSUE: These .env files are tracked in Git: {tracked_env}"
            )
...
def main():
    ...
    results.append(
        run_command(
            ".venv/bin/pip-audit --desc", "Checking for vulnerable dependencies"
        )
    )
    ...
    results.append(
        ...
        == False
    )  # We want this to fail (packages should be gone)
